        ...
        private bool UsePlaceholderChannel(string workerRuntime)
        {
            if (!string.IsNullOrEmpty(workerRuntime))
            {
                    ...
                    || string.Equals(workerRuntime, RpcWorkerConstants.PythonLanguageWorkerName, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var ret = _applicationHostOptions.CurrentValue.IsFileSystemReadOnly && !_environment.IsV2CompatibileOnV3Extension();
                    _logger.LogCritical("UsePlaceholderChannel: {0}", ret);
                    return ret;
                }
                return true;
            }
